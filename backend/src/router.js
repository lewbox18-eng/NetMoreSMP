const { HttpError, readJsonBody, readRawBody, sendJson, sendNoContent, getCorsHeaders, getRequestUrl } = require("./utils/http");
...
function createRouter({ config, services }) {
  ...
  return async function router(req, res) {
    ...
    if (req.method === "POST" && pathname === "/api/payments/checkout") {
      const body = await readJsonBody(req);
      const payment = await services.paymentService.startCheckout(body.orderId);
      sendJson(res, 200, { ok: true, payment }, corsHeaders);
      return;
    }
    ...
    if (req.method === "POST" && pathname === "/api/webhooks/stripe") {
      ...
      const result = await services.paymentService.handleStripeWebhook(rawBody, signature);
